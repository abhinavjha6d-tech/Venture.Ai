...
export default function Home() {
  ...
  const handleSendMessage = async (e?: React.FormEvent, customText?: string, overrideFile?: File | null) => {
    ...
    try {
      ...
      You can listen to audio notes, view images, and analyze uploaded PDFs or data files. 
            ...
            {attachment && (
              <div className="flex items-center justify-between bg-purple-950/60 border border-purple-800/60 p-3 rounded-2xl mb-3 text-xs shadow-lg">
                <div className="flex items-center gap-3 overflow-hidden">
                  {previewUrl ? (
                    <img src={previewUrl} alt="Preview" className="w-10 h-10 object-cover rounded-lg border border-purple-700/50" />
                  ) : (
                    <div className="w-10 h-10 bg-purple-900/50 rounded-lg flex items-center justify-center text-purple-300 font-bold text-xs uppercase">
                      FILE
                    </div>
                  )}
                ...
                <button 
                  ...
                  className="bg-red-500/20 hover:bg-red-500/30 text-red-300 p-1.5 rounded-xl border border-red-500/30 transition"
