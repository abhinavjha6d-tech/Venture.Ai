...
export default function Home() {
  ...
  const [chatHistory, setChatHistory] = useState([
    { id: "1", title: "Startup Brainstorming & MRR Growth" },
    { id: "2", title: "Unit Economics & CAC Optimization" }
  ]);
  ...
  const handleSendMessage = async (e?: React.FormEvent, customText?: string, overrideFile?: File | null) => {
    ...
    try {
      const systemInstruction = `You are Venture AI, founded by Abhinav Jha, Dev Kamra, and Anurag Gulati. You are a friendly, sharp, and supportive startup advisor and decision helper. Current user telemetry context: Target MRR is ₹${targetMrr}, Active Users are ${activeUsers}, CAC is ₹${cac}, ARPU is ₹${arpu}.`;
      ...
      contents.push({ role: "user", parts: partsArray });
  ...
  return (
    <div className="flex h-screen bg-[#070514] text-white font-sans overflow-hidden selection:bg-purple-500 selection:text-white">
      ...
      {session && (
        <Sidebar 
          ...
          onNewChat={() => setMessages([{ role: "assistant", content: "New strategy session started. What's on your mind?" }])}
          onSelectChat={(id) => setMessages([{ role: "assistant", content: `Loaded session ${id}. Let's continue.` }])}
        />
      )}

      <main className="flex-1 flex flex-col h-screen overflow-y-auto p-4 md:p-8">
        
        <header className="flex justify-between items-center mb-6 border-b border-purple-900/40 pb-4">
        ...
        {session ? (
                  ...
                  {attachment && !isUploadingFile && (
                    <div className="flex items-center justify-between bg-purple-950/60 border border-purple-800/60 p-3 rounded-2xl mb-3 text-xs shadow-lg">
                      <div className="flex items-center gap-3 overflow-hidden">
                        {previewUrl ? <img src={previewUrl} alt="Preview" className="w-10 h-10 object-cover rounded-lg border border-purple-700/50" /> : <div className="w-10 h-10 bg-purple-900/50 rounded-lg flex items-center justify-center text-purple-300 font-bold text-xs uppercase">FILE</div>}
                      ...
                      <button onClick={() => { setAttachment(null); setPreviewUrl(null); }} className="bg-red-500/20 hover:bg-red-500/30 text-red-300 p-1.5 rounded-xl border border-red-500/30 transition"><X size={14} /></button>
                    </div>
                  )}

                  <form onSubmit={(e) => handleSendMessage(e)} className="relative mt-auto flex items-center gap-2">
                    <input type="file" ref={fileInputRef} onChange={handleFileSelect} className="hidden" />
                    <button type="button" onClick={() => fileInputRef.current?.click()} disabled={isLoading || isUploadingFile} className="bg-[#0a071e] hover:bg-purple-950/50 border border-purple-900/50 p-3 rounded-xl text-purple-400 transition disabled:opacity-50"><Paperclip size={18} /></button>
                    <button type="button" onClick={toggleRecording} disabled={isLoading || isUploadingFile} className={`border p-3 rounded-xl transition disabled:opacity-50 ${isRecording ? "bg-red-500/20 border-red-500 text-red-400 animate-pulse" : "bg-[#0a071e] hover:bg-purple-950/50 border-purple-900/50 text-purple-400"}`}><Mic size={18} /></button>
                    <div className="relative flex-1">
                      <input type="text" value={input} onChange={(e) => setInput(e.target.value)} disabled={isLoading || isUploadingFile} placeholder={isRecording ? "Recording voice note..." : "Chat naturally in English, Hindi or Hinglish..."} className="w-full bg-[#0a071e] border border-purple-900/50 rounded-xl py-3 pl-4 pr-12 text-sm text-white placeholder-gray-500 focus:outline-none focus:border-purple-500 transition disabled:opacity-50" />
                      <button type="submit" disabled={isLoading || isUploadingFile} className="absolute right-2 top-1/2 -translate-y-1/2 bg-gradient-to-r from-purple-600 to-pink-500 hover:opacity-90 p-2 rounded-lg text-white transition shadow-md shadow-purple-600/40 disabled:opacity-50"><Send size={16} /></button>
                    </div>
                  </form>
                </div>
                ...
                <div className="flex flex-col gap-6">
                  <div className="bg-[#120F29]/90 border border-purple-900/50 rounded-3xl p-6 shadow-xl backdrop-blur-xl">
                    <div className="flex justify-between items-center mb-4">
                      ...
                      <span className="bg-purple-500/10 border border-purple-500/30 text-purple-300 text-[10px] px-3 py-1 rounded-full font-bold uppercase tracking-wider">OPTIMIZED</span>
                    </div>

                    <div className="grid grid-cols-2 gap-3 mt-4">
                      <div className="bg-[#0a071e] border border-purple-900/40 p-3.5 rounded-2xl"><span className="text-[11px] text-gray-400 block mb-1">Target MRR</span><span className="text-lg font-black text-white">₹{targetMrr.toLocaleString()}</span></div>
                      <div className="bg-[#0a071e] border border-purple-900/40 p-3.5 rounded-2xl"><span className="text-[11px] text-gray-400 block mb-1">Active Users</span><span className="text-lg font-black text-white">{activeUsers.toLocaleString()}</span></div>
                      <div className="bg-[#0a071e] border border-purple-900/40 p-3.5 rounded-2xl"><span className="text-[11px] text-gray-400 block mb-1">CAC</span><span className="text-lg font-black text-pink-400">₹{cac}</span></div>
                      <div className="bg-[#0a071e] border border-purple-900/40 p-3.5 rounded-2xl"><span className="text-[11px] text-gray-400 block mb-1">ARPU</span><span className="text-lg font-black text-purple-400">₹{arpu}</span></div>
                    </div>
                  </div>

                  <div className="bg-[#120F29]/90 border border-purple-900/50 rounded-3xl p-6 shadow-xl backdrop-blur-xl">
                    <div className="flex justify-between items-center mb-4">
                      <div className="flex items-center gap-2"><Activity size={16} className="text-purple-400" /><span className="text-xs font-bold tracking-wider text-white uppercase">REVENUE TRAJECTORY MODEL</span></div>
                    ...
                    <div className="h-36 w-full relative flex items-end pt-4">
                      <svg className="w-full h-full overflow-visible" viewBox="0 0 500 150" preserveAspectRatio="none">
                        ...
                        <path d={`M 0,${Math.max(20, 140 - (targetMrr / 800))} Q 125,${Math.max(10, 110 - (activeUsers / 80))} 250,${Math.max(10, 80 - (targetMrr / 1500))} T 500,${Math.max(5, 40 - (targetMrr / 2000))} L 500,150 L 0,150 Z`} fill="url(#grad)" />
                        <path d={`M 0,${Math.max(30, 130 - (targetMrr / 900))} Q 125,${Math.max(15, 95 - (activeUsers / 70))} 250,${Math.max(10, 65 - (targetMrr / 1400))} T 500,${Math.max(5, 25 - (targetMrr / 1800))}`} fill="none" stroke="#ec4899" strokeWidth="3" />
                        <path d={`M 0,${Math.max(50, 140 - (targetMrr / 1000))} Q 125,${Math.max(25, 120 - (activeUsers / 90))} 250,${Math.max(20, 90 - (targetMrr / 1600))} T 500,${Math.max(10, 50 - (targetMrr / 2200))}`} fill="none" stroke="#22d3ee" strokeWidth="2.5" strokeDasharray="4 4" />
                      </svg>
                    </div>
                  </div>
                  ...
                  <div className="bg-[#120F29]/90 border border-purple-900/50 rounded-3xl p-6 shadow-xl backdrop-blur-xl">
                    ...
                    <div className="space-y-4">
                      <div className="bg-[#0a071e] p-4 rounded-2xl border border-purple-900/30">
                        <div className="flex justify-between items-center mb-2"><label className="text-xs font-semibold text-gray-300">Target MRR</label><span className="text-purple-400 font-bold text-sm">₹{targetMrr}</span></div>
                        <input type="range" min="1000" max="100000" step="1000" value={targetMrr} onChange={(e) => setTargetMrr(Number(e.target.value))} className="w-full accent-purple-500 cursor-pointer" />
                      </div>
                      <div className="bg-[#0a071e] p-4 rounded-2xl border border-purple-900/30">
          ...
          <div className="max-w-md mx-auto mt-16 bg-[#120F29]/90 border border-purple-900/60 rounded-3xl p-8 shadow-2xl backdrop-blur-2xl">
            <div className="text-center mb-6">
              <h2 className="text-2xl font-black text-white mb-2">{authMode === "signin" ? "Welcome Back" : "Create Account"}</h2>
            ...
            <form onSubmit={handleAuth} className="space-y-4">
              <div>
                ...
                <input type="email" required value={email} onChange={(e) => setEmail(e.target.value)} placeholder="name@example.com" className="w-full bg-[#0a071e] border border-purple-900/50 rounded-xl px-4 py-3 text-sm text-white placeholder-gray-500 focus:outline-none focus:border-purple-500 transition" />
              </div>
              <div>
                <label className="block text-xs font-semibold text-gray-300 mb-1">Password</label>
                <input type="password" required value={password} onChange={(e) => setPassword(e.target.value)} placeholder="••••••••" className="w-full bg-[#0a071e] border border-purple-900/50 rounded-xl px-4 py-3 text-sm text-white placeholder-gray-500 focus:outline-none focus:border-purple-500 transition" />
              </div>
              ...
              <button type="submit" className="w-full bg-gradient-to-r from-purple-600 to-pink-500 hover:opacity-90 text-white font-semibold py-3 rounded-xl transition shadow-lg shadow-purple-600/30 text-sm mt-2">{authMode === "signin" ? "Sign In" : "Sign Up"}</button>
            </form>
            <div className="text-center mt-6">
              <button type="button" onClick={() => { setAuthMode(authMode === "signin" ? "signup" : "signin"); setAuthError(""); setAuthMessage(""); }} className="text-xs text-purple-400 hover:text-purple-300 transition">{authMode === "signin" ? "Don't have an account? Sign up" : "Already have an account? Sign in"}</button>
            </div>
          </div>
        )}
      </main>
